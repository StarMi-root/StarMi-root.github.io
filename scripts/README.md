...
```bash
# 快速运行（推荐先试这个）：http://localhost:8080
bash scripts/serve-local.sh

# 换个端口
bash scripts/serve-local.sh --port 9000

# 修改过代码后先重新构建再运行
bash scripts/serve-local.sh --build

# 装成开机自启的常驻服务（nginx，端口 80）
sudo bash scripts/serve-local.sh --system
```
...
| 地址 | 谁能访问 |
|---|---|
| `http://localhost:8080` | 只有这台电脑 |
| `http://局域网IP:8080` | 同一 WiFi / 网段下的手机、电脑 |
| `http://100.x.x.x:8080` | 同一 **Tailscale** 网络（Tailnet）里的任意设备，跨网也行 |

> 局域网设备打不开？多半是防火墙：`sudo ufw allow 8080`。
> 快速模式关掉终端服务就停；`--system` 模式则常驻并开机自启（`sudo systemctl stop nginx` 停止）。

## 让公网（全世界）访问本机网站

本机服务默认只有内网能访问，想让任何人都能打开，三种办法：

### 办法 A：Tailscale Funnel（你已装了 Tailscale，最省事）

服务跑在 8080 端口时，另开一个终端执行：

```bash
tailscale funnel 8080
```

它会输出一个 `https://你的机器名.tail***.ts.net` 的**公网 HTTPS 地址**，
任何人都能打开，关掉该终端即失效。
（首次使用需在 [Tailscale 管理后台](https://login.tailscale.com/admin) 开启 HTTPS 与 Funnel 开关，免费。）

### 办法 B：Cloudflare 临时隧道（不用账号）

```bash
# 安装：sudo apt install cloudflared 或到 github.com/cloudflare/cloudflared 下载
cloudflared tunnel --url http://localhost:8080
```

同样输出一个 `https://xxx.trycloudflare.com` 的临时公网地址，关终端即失效，适合临时演示。

### 办法 C：路由器端口转发（长期、不依赖第三方）

登录路由器后台，把 WAN 侧 80/8080 端口转发到本机局域网 IP，
再用你家宽带的公网 IP 访问。多数家庭宽带 80 端口被运营商封掉且 IP 会变，
不推荐，优先用 A 或 B。
